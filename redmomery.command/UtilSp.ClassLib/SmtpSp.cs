...
namespace UtilSp.ClassLib
{
    public class SmtpSp
    {
        ...
        {
            string back = receive();
            return back.Substring(0, 3).Equals(SMTP_BACKS.SERVER_READY_OK);
        }
        #endregion
        ...
        {
            byte[] attachmentData =new byte[0]; //FileSp.readFileBytes(attachment);/
        ...
        {
            string header = "--" + boundary_pro + "\r\n"
                + "Content-type:text/" + (ishtml ? "html" : "plain")
                 +";charset=" + encoding_content_pro.BodyName + "\r\n"//注意content-type的传输严格按照规定来
                 + "Content-Transfer-Encoding:base64\r\n";
            socketSend(header + "\r\n");
        }
