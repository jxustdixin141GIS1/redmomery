...
namespace redmomery.DAL
{
	partial class meetingtableDAL
	{
		...
		private static meetingtable ToModel(DataRow row)
		{
			...
			model.Ttime = (DateTime)(row["Ttime"]==null?row["Ttime"]:DateTime.Parse("9999/12/31"));
			model.local = (string)(row["local"]==null?"":row["local"].ToString());
			model.contentTitle = (string)(row["contentTitle"]==null?"":row["contentTitle"].ToString());
			model.context = (string)(row["context"]==null?"":row["context"].ToString());
			model.meetTime = (DateTime)(row["meetTime"]==null?row["meetTime"]:DateTime.Parse("9999/12/31"));
            model.img = (string)(row["img"] == null ? "" : row["img"].ToString());
			model.vnum = (int)(row["vnum"]==null?0:int.Parse(row["vnum"].ToString()));
			model.isCheck = (int)(row["isCheck"]==null?0:int.Parse(row["isCheck"].ToString()));
